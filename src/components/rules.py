...
class Rules:
    ...
    @staticmethod
    def group(arg: str) -> bool:
        """
        Un group est une chaîne de caractères qui commence par un 'g' suivi d'un entier positif.
        """
        try:
            arg_str = str(arg)
            return arg_str.startswith('g') and Rules.integer(arg_str[1:])
        except Exception:
            return False
